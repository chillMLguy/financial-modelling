...
for fold, (tr_idx,te_idx) in enumerate(walk_forward_split(n, n, test_size), start = 1):
    ...
    X_te = scaler.transform(X_te_2d).reshape(X_te.shape[0], X_te.shape[1], X_te.shape[2])

    model = LSTMmodel(n_features = X_tr.shape[2], hidden_lstm_units= lstm_neurons, dropout=dropout)
    model = train(model, X_tr.astype(np.float32), y_tr.astype(np.float32), val_split = test_size, epochs = epochs, batch_size=batch_size, lr=lr)


    y_hat = predict(model, X_te.astype(np.float32), y_te.astype(np.float32), device = 'cuda')
...
signal = (preds > sgn_threshold).astype(int)        #flag


bh_curve = buy_and_hold_dollars(data["ret"], initial=capital, fee_buy=fee_buy, fee_sell=fee_sel)
lstm_curve = backtest_daily_full_invest(data["ret"], signal, fee_buy=fee_buy, fee_sell=fee_sel, initial=capital)
